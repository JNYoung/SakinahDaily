...
require_text "$push_audit_doc" 'resolved, fallback route used'
...
require_text "$notification_tap_listener" '_localPushResolutionChangeType'
...
require_text "$notification_tap_service_test" 'resolutionOutcome'
...
require_text "$notification_tap_route_test" 'fallback resolution outcome'
...
local_push_resolution,complete,"NotificationTapService carries resolutionOutcome and NotificationTapRouteListener tracks local push payload resolution outcomes","notification_tap_service_test.dart and notification_tap_route_listener_test.dart","local_push_resolution_result: resolved|fallback_route_used|direct_route_fallback|missing_content|malformed_payload","No raw payload, route, content ID, prayer name, lock-screen body, or religious text"
...
local_push_resolution_result,local push payload resolution,"foreground, background, and cold-start local push payload parsing and content resolution","content_type|source=local_notification|change_type","payload|route|content_id|session_id|prayer_name|reminder_time|body|quran_arabic_text|women_ibadah_status|feedback_text","Do local push payloads resolve, use fallback, miss content, or fail parsing without exposing raw payload data? Expected change_type values: resolved|fallback_route_used|direct_route_fallback|missing_content|malformed_payload"
EOF
...
local_push_resolution_result,content_type|source|change_type,record_manually,record_manually,pending_manual_observation,No tester personal data; verify change_type values resolved|fallback_route_used|direct_route_fallback|missing_content|malformed_payload
EOF
...
- `notification_tap_result` 记录最终点击打开结果；`local_push_resolution_result` 记录 payload/content 解析结果。解析结果和点击打开结果分离，`local_push_resolution_result.change_type` 应覆盖 `resolved|fallback_route_used|direct_route_fallback|missing_content|malformed_payload`。
