...
import { isAgentCandidate } from "../domain/schemas.js";
...
export class AgentRunsRoute {
  ...
  async handle(req: IncomingMessage, res: ServerResponse): Promise<boolean> {
    ...
    if (req.method === "GET" && reviewMatch) {
      ...
      writeJson(
        res,
        run ? 200 : 404,
        run ? { run, candidates: run.candidates } : { error: "run_not_found" }
      );
      return true;
    }
    ...
    if (req.method === "POST" && qaMatch) {
      ...
      const flags = candidate ? collectSafetyFlags(candidate, source) : ["candidate_not_found"];
    ...
    if (req.method === "POST" && promoteMatch) {
      ...
      const source = this.contentRepository.getSourceItem(candidate.sourceItemId);
      const flags = collectSafetyFlags(candidate, source);
      if (flags.length > 0 || !isAgentCandidate(candidate)) {
        writeJson(res, 422, { candidateId: candidate.candidateId, promoted: false, flags });
        return true;
      }
      const promoted = { ...candidate, status: "cms_draft" as const };
      this.runRepository.updateCandidate(promoted);
      writeJson(res, 200, {
  ...
  createRun(body: CreateAgentRunRequest = {}) {
    ...
    const selected = selectCandidateSources(this.contentRepository.listSourceItems(), {
      language,
      ritualMoment,
      recentClusterIds: body.recentClusterIds,
      womenLockScreenSafe: body.womenLockScreenSafe ?? true
    });
