...
| P1 | Privacy-safe analytics plan | Product metrics are defined in PRD, but no measurement exists by default. | GA4-compatible event contract and Firebase Analytics adapter exist; collection is default-off, sanitized, and disabled in store screenshot mode. | Add Firebase project configuration and consent/Data Safety approval before enabling telemetry for beta or production. |
