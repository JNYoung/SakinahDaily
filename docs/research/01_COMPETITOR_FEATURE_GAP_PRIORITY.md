...
| P0 | Minimal Quran reader slice | Quran Majeed/Muslim Pro normalize full Quran access. Sakinah can stay session-led, but saved Quran routes need credible depth. | Quran entry/detail, local browse/search, and previous/next detail navigation exist for approved seed ayahs only; no Surah/Juz/full corpus route; seed source labels are now visible but corpus depth remains limited. | Add an approved limited Quran slice for session verses and keep the simple local browse/search/navigation constrained to reviewed content. Defer full Quran reader until source corpus and translation rights are approved. |
...
`SAKINAH_REVIEWED_CONTENT_PACK_OWNER_ASSIGNED=true`, plus completed
...
`SAKINAH_REVIEWED_CONTENT_OWNER_EVIDENCE` CSVs with no template placeholders or
