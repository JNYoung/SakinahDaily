...
| Women's Ibadah Mode state | Yes, if enabled | Yes | No | No | Optional | Local privacy-aware reminder behavior, discreet on-screen visibility preference, and local UI recommendation adjustments | Settings > Privacy > Delete local data | High sensitivity, local-only in MVP |
...
| Default-off analytics events | Only when explicitly enabled, configured, and opted in | No durable client storage by default | Yes, only when `SAKINAH_ANALYTICS_ENABLED=true`, Firebase configuration is present, and the user opts in | Firebase/Google as analytics processor when enabled | Optional | Retention and prayer/session usage analysis after review | Turn off usage analytics in Privacy Center; remote deletion is future work | Whitelisted GA4 events only; Home/prayer checklist retention fields are aggregate counts only; prayer location setup analytics keeps only coarse method/source/change type and does not send coordinates, manual place labels, timezone IDs, or routes; Qibla analytics keeps only coarse method/source fields and does not send coordinates, place labels, or bearing degrees; Daily Session reminder analytics keeps only enabled state/source/change type and does not send exact reminder time; no exact coordinates, Women's Ibadah Mode exact status, feedback text, religious text, exact prayer completion names/timestamps, names, or email |
...
- Discreet privacy mode for Women's Ibadah Mode may hide Home and Daily Session
  support copy locally; the preference must stay on device and must not be sent
  with remote content requests or analytics parameters.
...
- Analytics for prayer location setup must not include exact coordinates,
  manual place labels, timezone IDs, or routes.
