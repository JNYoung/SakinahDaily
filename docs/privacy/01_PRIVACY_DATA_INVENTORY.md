...
| Prayer location preset / manual location | Yes, when selected | Yes | No by default | No | Optional | Prayer-time calculation | Settings > Privacy > Delete local data | Location concern if exact GPS permission is added later |
...
- Delete local data must clear preferences, cached manifests, cached bundles,
  revoked content IDs, and scheduled local reminders.
