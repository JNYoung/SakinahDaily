...
- D1 / D7 Retention.
- Push Tap Outcome Health from aggregate tester feedback by default, or
  `notification_tap_result` DebugView evidence when reviewed analytics QA is
  explicitly approved.
...
Notification Settings QA smoke buttons should appear only as
`notification_smoke_test_result` with coarse content type,
`source=notification_settings_qa`, and coarse result type, so the operator can
verify local notification scheduling or permission denial without storing raw
...
Local notification tap handling, including cold-start launches from local
...
identity, Women's Ibadah Mode exact status, lock-screen body copy, or religious
text.
