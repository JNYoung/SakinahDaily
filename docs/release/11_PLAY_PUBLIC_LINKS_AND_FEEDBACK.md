...
Current blocker as of 2026-06-21: the candidate privacy policy URL
`https://misbahdaily.com/privacy` is not publicly reachable over HTTPS yet
(`curl` failed with `LibreSSL SSL_connect: SSL_ERROR_SYSCALL`), so
`scripts/verify_google_play_public_links.sh` cannot pass without network
skipping. Publish the reviewed page from `build/play-public-links/privacy/`
before treating the Play upload preflight as ready.
...
```sh
SAKINAH_PRIVACY_POLICY_URL=https://misbahdaily.com/privacy \
SAKINAH_PLAY_TESTING_FEEDBACK=support@misbahdaily.com \
...
- The Misbah Daily Alpha Testers invitation copy when recruiting testers.
